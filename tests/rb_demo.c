...
// static int kth_val(Node* node, int k, Node* NIL) {
//     int val = 0;
//     while (node != NIL) {
//         if (node->left->size >= k) node = node->left;
//         else if (node->left->size == k - 1) return node->val;
//         else k -= node->left->size + 1, node = node->right;
//     }
//     return val;
// }
...
// static int upper_bound(Node* node, int val, Node* NIL) {
//     int suc_val = val;
//     while (node != NIL) {
//         if (node->val > val) suc_val = node->val, node = node->left;
//         else node = node->right;
//     }
//     return suc_val;
// }
...
int main() {
    int n, m;
    scanf("%d%d", &n, &m);
    RB_Tree* tree = rb_tree_create(n + m);
    for (int i = 1, x; i <= n; i++) {
        scanf("%d", &x);
        rb_tree_insert(tree, x);
    }
    int ans = 0, last = 0;
    for (int i = 1, opt, x; i <= m; i++) {
        ...
        case 1:
            ...
            break;
        case 2:
            rb_tree_erase(tree, x);
            break;
        case 3:
            last = rb_tree_rank(tree, x);
            ans ^= last;
            break;
        case 4:
            last = rb_tree_kth_val(tree, x);
            ans ^= last;
            break;
        case 5:
            last = rb_tree_prev_val(tree, x);
            ans ^= last;
            break;
        case 6:
            last = rb_tree_next_val(tree, x);
            ans ^= last;
            break;
        default:
            break;
